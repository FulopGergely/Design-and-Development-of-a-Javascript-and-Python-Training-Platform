...
onMounted(() => {
  initTest().then(() => {
    //előbb szerverről lekérjük az adatot, aztán init a többi szükséges adatokat
    let score = 0
...
function modyfiCode(codes) {
  //modosítjuk, hogy a teszkitöltő csak a függvény törzset lássa. Betöltés után modosítjuk az egészet, így könnyebb lesz később a kezelése.
  ...
  codes.forEach((code) => {
    try {
      ...
      if (
        code.programmingLanguageName &&
        code.programmingLanguageName.value == 'javascript' &&
        code.code
      ) {
        //js kód, és van kód beírva
      ...
      if (
        code.programmingLanguageName &&
        code.programmingLanguageName.value == 'python' &&
        code.code
      ) {
    ...
    <div v-if="store.getters.getLoading">
      ...
      <div v-else>
        ...
        <div v-for="task in store.getters.getTestSheet.task" :key="task.side">
          <div
            v-if="task.side == store.getters.getCurrentTestSide"
              ...
              <div>
                ...
                >
                  <div class="flex justify-content-end flex-wrap mr-5">
                    {{ store.getters.getScoreBySide }} pont ( {{ scoreAchieved }} /
                    {{ totalScore }} )
                  </div>
                  <div
                    class="m-5 task-text"
                  ...
                  <div v-if="task.programmingLanguageName">
                    <CodeRunnerTestFiller
                      v-if="
                        task.programmingLanguageName.value == 'javascript' ||
                        task.programmingLanguageName.value == 'python'
                      "
                      v-model:taskCode="task.code"
                      :selectLanguage="task.programmingLanguageName.value"
                      :tests="task.tests"
                    />
                  </div>
                  <div
                    v-if="store.getters.getDisplayTest"
                    class="flex justify-content-center flex-wrap"
                  >
                    <div class="border-round surface-border mt-5 mb-3 p-4 w-full">
